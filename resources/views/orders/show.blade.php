...
<div class="row">
    <div class="col-md-12">
            ...
            <div class="col-md-6">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        @else
                            <p class="text-muted">No supplier assigned yet. Order is pending.</p>
                        @endif
                    </div>
                </div>
            </div>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $item->product->sku }}</br>
                                    <small>{{ $item->product->category->name ?? '' }}</small>
                                </td>
                                <td>{{ $item->quantity }}</br>
                                    <small>{{ $item->quantity > 1 ? 'items' : 'item' }}</small>
                                </td>
                                <td>৳{{ number_format($item->price, 2) }}</td>
                                <td>৳{{ number_format($item->quantity * $item->price, 2) }}</br>
                        ...
                        <tfoot>
                            <tr class="table-active">
                                ...
                                <td><strong>৳{{ number_format($order->total_amount - $order->delivery_charge, 2) }}</strong></td>
                            </tr>
                            <tr class="table-active">
                                <td colspan="4" class="text-end"><strong>Delivery Charge:</strong></td>
                                <td><strong>৳{{ number_format($order->delivery_charge, 2) }}</strong></td>
                            </tr>
                            <tr class="table-active">
                            ...
                            <tr class="table-active">
                                <td colspan="4" class="text-end"><strong>Commission (1%):</strong></td>
                                <td><strong class="text-info">৳{{ number_format($order->commission_amount, 2) }}</strong></td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
        ...
        @if(auth()->user()->hasRole('outlet-owner') && $order->supplier_outlet_id == null && $order->status == 'pending')
        <div class="card mt-4 bg-light">
            <div class="card-body text-center">
                <h5 class="text-warning">This order is pending!</h5>
                <p>You can accept or reject this order.</p>
                <button onclick="acceptOrder({{ $order->id }})" class="btn btn-success">
                    <i class="fas fa-check"></i> Accept Order
                </button>
                <button onclick="rejectOrder({{ $order->id }})" class="btn btn-danger">
...
<script>
    function acceptOrder(orderId) {
        if(confirm('Accept this order?')) {
            $.ajax({
                url: '/outlet/orders/' + orderId + '/accept',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
    ...
    function rejectOrder(orderId) {
        if(confirm('Reject this order?')) {
            $.ajax({
                url: '/outlet/orders/' + orderId + '/reject',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
